import { assessGoalInput } from "@/lib/goals/concreteness-gate";
...
/** Short, actionable goal title — not a sentence or timeframe. */
export function isBadGoalTitle(raw: string): boolean {
...
export function validateGoalTitle(raw: string): {
  valid: boolean;
  title: string;
  error?: string;
  suggestions?: string[];
} {
  const assessment = assessGoalInput(raw);
  if (!assessment.valid) {
    return {
      valid: false,
