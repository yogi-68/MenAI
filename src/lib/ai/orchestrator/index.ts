...
import type { OrchestratorInput,  PipelineContext, UserProfile, EmotionAnalysis } from "./types";
...
async function _orchestrateStreamingInternal(input: OrchestratorInput): Promise<{
  ...
  // Honour the routers decision. This used to be followed by an
  // unconditional `modelConfig.model = COACH_CHAT_MODEL`, which discarded the
  // tiering entirely and put every turn on the expensive model.
